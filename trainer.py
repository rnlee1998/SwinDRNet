...
class SwinDRNetTrainer():
    ...
    def forward(self, sample_batched, mode):
        ...
        if mode=='single_test':
            ...
            pred_ds, pred_ds_initial, confidence_sim_ds, confidence_initial = self.model(rgbs, sim_ds)
            return pred_ds
    ...
    def save_iter_depth_restoration_metrics(self, labels, outputs, masks, mode):
        ...
        if mode == 'train':
            ...
            self.loss_train_epoch_depth_restoration['total'] =self.loss_train_epoch_depth_restoration['total'] + \
            ...
            if self.depth_restoration_train_with_masks:
                ############################### depth restoration loss #########################################
                self.loss_train_epoch_depth_restoration['instance'] = self.loss_train_epoch_depth_restoration['instance']+ \
                                                self.loss_train_iter_depth_restoration['instance'].item() / num_samples_train
                self.loss_train_epoch_depth_restoration['background'] = self.loss_train_epoch_depth_restoration['background'] + \
            ...
            ############################### depth restoration loss #########################################
            self.loss_val_epoch_depth_restoration['total'] = self.loss_val_epoch_depth_restoration['total'] + \
            ...
            if self.depth_restoration_train_with_masks:
                ############################### depth restoration loss #########################################
                self.loss_val_epoch_depth_restoration['instance'] = self.loss_val_epoch_depth_restoration['instance'] + \
                                                self.loss_val_iter_depth_restoration['instance'].item() / num_samples_val
                self.loss_val_epoch_depth_restoration['background'] = self.loss_val_epoch_depth_restoration['background'] + \
    ...
    def save_iter_sem_seg_metrics(self, labels, outputs, mode):
        ...
        # statistics
        if mode == 'train':
            self.loss_train_epoch_sem_seg = self.loss_train_epoch_sem_seg + self.loss_train_iter_sem_seg.item() / num_samples_train
            ...
            self.loss_val_epoch_sem_seg =self.loss_val_epoch_sem_seg + self.loss_val_iter_sem_seg.item() / num_samples_val
    ...
    def save_iter_coord_metrics(self, mode):
        ...
        num_samples_val = len(self.val_loader)
        if mode == 'train':
            self.loss_train_epoch_coord =self.loss_train_epoch_coord + self.loss_train_iter_coord.item() / num_samples_train
            self.writer.add_scalar('Train/loss/Train_Loss_Coord', self.loss_train_iter_coord.item(), self.total_iter_num)
        else:
            self.loss_val_epoch_coord =self.loss_val_epoch_coord + self.loss_val_iter_coord.item() / num_samples_val
    ...
    def inference(self):
        ...
        for i, batch in enumerate(tqdm(self.val_loader)):
            ...
            sample_batched = self.transfer_to_device(batch)
            with torch.no_grad():
                # syn_depth_list.append(sample_batched['syn_depth'].mean().cpu())
                # syn_depth_max.append(sample_batched['syn_depth'].max().cpu())
                # syn_depth_min.append(sample_batched['syn_depth'].min().cpu())
                # outputs_depth, normal_labels, normal_pred, confidence_sim_ds, confidence_initial, \
                #     pred_sem_seg, pred_coord = self.forward(cache, mode='val')                
                outputs_depth, normal_labels, normal_pred, confidence_sim_ds, confidence_initial = self.forward(sample_batched, mode='val')
                
                # import open3d as o3d
                # points, color = RGBD2Point(outputs_depth[0,0,:,:].cpu(),sample_batched['rgb'].squeeze().cpu())
        ...
        # print(np.mean(syn_depth_list),np.mean(syn_depth_max),np.mean(syn_depth_min))
                # outputs_depth_list.append(outputs_depth)
                ...
                # cv2.imwrite(os.path.join('/data/ran1998.li/workdir/DREDS-main/SwinDRNet/results/DREDS_CatKnown/depth/','%s.png'%(i)), outputs_depth[0,0,:,:])
        ...
        return metrics_instance, metrics_background, outputs_depth_list
